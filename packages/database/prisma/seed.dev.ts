...
async function main() {
  ...
  console.log('🌱 Iniciando seed de desenvolvimento (destrutivo)...');

  const allowDestructiveInProd = process.env.ALLOW_DESTRUCTIVE_SEED === 'true'
  if (process.env.NODE_ENV === 'production' && !allowDestructiveInProd) {
    throw new Error(
      'Seed destrutivo bloqueado em produção. Use db:seed:prod ou defina ALLOW_DESTRUCTIVE_SEED=true conscientemente.',
    )
  }
