...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const followed: string[] = layout?.community?.followedTopics || []
...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const followed: string[] = Array.isArray(community.followedTopics) ? community.followedTopics : []
    if (!followed.includes(params.id)) followed.push(params.id)
    const newLayout = { ...layout, community: { ...community, followedTopics: followed } }
    await prisma.userPreferences.update({ where: { id: prefs.id }, data: { layout: newLayout as any } })
    return NextResponse.json({ followed: true })
  } catch (error) {
    console.error('Erro ao seguir tópico:', error)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const community = layout.community || {}
    const followed: string[] = Array.isArray(community.followedTopics) ? community.followedTopics : []
    const newFollowed = followed.filter((t) => t !== params.id)
    const newLayout = { ...layout, community: { ...community, followedTopics: newFollowed } }
    await prisma.userPreferences.update({ where: { id: prefs.id }, data: { layout: newLayout as any } })
    return NextResponse.json({ followed: false })
  } catch (error) {
    console.error('Erro ao deixar de seguir tópico:', error)
    return NextResponse.json({ error: 'Erro interno' }, { status: 500 })
  }
}
