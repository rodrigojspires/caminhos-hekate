...
import { applyMove, isCompleted, START_INDEX, getHouseByNumber } from '@hekate/mahalilah-core'

dotenv.config({ path: path.resolve(__dirname, '../../../.env') })

const PORT = Number(process.env.MAHALILAH_REALTIME_PORT || 4010)
const CLIENT_URL = process.env.NEXT_PUBLIC_MAHALILAH_URL || process.env.NEXTAUTH_URL || 'http://localhost:3000'
const CLIENT_URLS = (process.env.MAHALILAH_CLIENT_URLS || CLIENT_URL)
  .split(',')
  .map((item) => item.trim())
  .filter(Boolean)
const SOCKET_SECRET = process.env.MAHALILAH_SOCKET_SECRET || process.env.NEXTAUTH_SECRET || 'mahalilah-secret'
...
const THERAPY_COOLDOWN_MS = Number(process.env.MAHALILAH_THERAPY_COOLDOWN_MS || 800)
...
const AI_TIPS_LIMITS: Record<string, number> = {
  ...
  SUBSCRIPTION_LIMITED: Number(process.env.MAHALILAH_TIPS_SUBSCRIPTION_LIMITED || 3)
}
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'Você é um terapeuta que usa o jogo Maha Lilah para gerar insights. Responda em português, com estrutura clara e tom acolhedor.'
        },
...
async function buildAiContext(roomId: string, participantId: string) {
  ...
  const path = moves
    .filter((move) => !(move.fromPos === 68 && move.toPos === 68 && move.diceValue !== 6))
    .map((move) => move.toPos)
    .reverse()
  ...
  return {
    ...
    currentHouse: houseMeta ? { number: houseMeta.number, title: houseMeta.title, description: houseMeta.description } : { number: currentHouse },
    path,
    ...
    recentMoves: moves.map((move) => ({
      ...
      from: move.fromPos,
      to: move.toPos,
      jump: move.appliedJumpFrom ? `${move.appliedJumpFrom}→${move.appliedJumpTo}` : null
    })),
...
function ensureConsentAccepted(participant: { consentAcceptedAt: Date | null }) {
  ...
  const room = await prisma.mahaLilahRoom.findUnique({
    ...
    include: {
      ...
      cardDraws: {
        orderBy: { createdAt: 'desc' },
        include: {
          drawnBy: { include: { user: { select: { id: true, name: true, email: true } } } }
        }
      }
    }
  })
...
async function rollInRoom(roomId: string, userId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    const currentState = playerStates.find((state) => state.participantId === currentParticipant.id)
    if (!currentState) {
    ...
    await tx.mahaLilahPlayerState.update({
      ...
      data: {
        ...
        rollCountUntilStart: move.hasStartedBefore ? undefined : { increment: 1 }
      }
    })

    const moveCount = await tx.mahaLilahMove.count({ where: { roomId: room.id } })
...
io.use((socket, next) => {
  try {
    ...
    const decoded = jwt.verify(token, SOCKET_SECRET) as { sub: string; email: string; name?: string; role?: string }
    socket.data.user = {
      id: decoded.sub,
      email: decoded.email,
      name: decoded.name,
...
io.on('connection', (socket: AuthedSocket) => {
  socket.on('room:join', async ({ code }: { code: string }, callback?: (payload: any) => void) => {
    try {
  ...
  socket.on('game:roll', async (_payload: any, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
      enforceCooldown(`roll:${socket.data.user.id}:${socket.data.roomId}`, ROLL_COOLDOWN_MS)
  ...
  socket.on('deck:draw', async ({ count, moveId }: { count: number; moveId?: string }, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
      enforceCooldown(`deck:${socket.data.user.id}:${socket.data.roomId}`, DECK_COOLDOWN_MS)
      const room = await prisma.mahaLilahRoom.findUnique({
      ...
      const participant = room.participants.find((p) => p.userId === socket.data.user?.id)
      if (!participant) throw new Error('Participante não encontrado')
      ensureConsentAccepted(participant)

      const drawCount = Math.min(Math.max(count || 1, 1), 3)
      const cards = Array.from({ length: drawCount }, () => Math.floor(Math.random() * 72) + 1)
      let safeMoveId: string | null = null

      if (moveId) {
        const move = await prisma.mahaLilahMove.findUnique({ where: { id: moveId } })
        if (!move || move.roomId !== room.id) throw new Error('Jogada inválida')
  ...
  socket.on('therapy:save', async ({ moveId, emotion, intensity, insight, body, microAction }: any, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
      enforceCooldown(`therapy:${socket.data.user.id}:${socket.data.roomId}`, THERAPY_COOLDOWN_MS)
      ...
      const move = await prisma.mahaLilahMove.findUnique({ where: { id: moveId } })
      if (!move || move.roomId !== socket.data.roomId) throw new Error('Jogada inválida')
      if (move.participantId !== participant.id) {
  ...
  socket.on('ai:tip', async ({ intention }: { intention?: string } = {}, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
      enforceCooldown(`ai:${socket.data.user.id}:${socket.data.roomId}`, AI_COOLDOWN_MS)
      const room = await prisma.mahaLilahRoom.findUnique({ where: { id: socket.data.roomId } })
      if (!room) throw new Error('Sala não encontrada')
      if (room.status !== 'ACTIVE') throw new Error('Sala não está ativa')
      ...
      if (room.orderId) {
        const order = await prisma.order.findUnique({ where: { id: room.orderId } })
      ...
      const usage = await prisma.mahaLilahAiUsage.upsert({
        where: { roomId_participantId: { roomId: room.id, participantId: participant.id } },
        update: {},
        create: { roomId: room.id, participantId: participant.id, tipsUsed: 0, tipsLimit }
      })
      ...
      await prisma.mahaLilahAiUsage.update({
        where: { id: usage.id },
  ...
  socket.on('ai:finalReport', async ({ intention }: { intention?: string } = {}, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
      enforceCooldown(`ai-final:${socket.data.user.id}:${socket.data.roomId}`, AI_COOLDOWN_MS)
      const room = await prisma.mahaLilahRoom.findUnique({ where: { id: socket.data.roomId } })
      if (!room) throw new Error('Sala não encontrada')
  ...
  socket.on('game:nextTurn', async (_payload: any, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
  ...
  socket.on('room:close', async (_payload: any, callback?: (payload: any) => void) => {
    try {
      if (!socket.data.user || !socket.data.roomId) throw new Error('Sala não selecionada')
